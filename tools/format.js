...
(function () {
  ...
  Object.keys(code).forEach((key) => {
    let graph = code[key].map((e) => {
      const id2date = (id) =>
        `${id.substring(40, 44)}-${id.substring(44, 46)}-${id.substring(
          46,
          48
        )}`;
        ...
        if (f.visible) {
          ...
          if (f.label.match(/[都道府県]$/)) {
            ...
            if (j["市区町村コード"] === null)
              j[
                "市区町村コード"
              ] = `http://data.e-stat.go.jp/lod/sac/C${f.code}`;
          }
        }
      }
    ...
    graph = (function () {
      ...
      graph.forEach((next) => {
        ...
        prev["メタデータ"]["日付"][1]["標準型日付"] =
          next["メタデータ"]["日付"][1]["標準型日付"];
    ...
    graph.forEach((e) => {
      ...
      const m = e["メタデータ"];
      if (m["日付"][1]["標準型日付"] === null) m["日付"] = m["日付"][0];
      if (m["参照"]["参照先"].length === 0) delete m["参照"];
      else if (m["参照"]["参照先"].length === 1)
        m["参照"]["参照先"] = m["参照"]["参照先"][0];
    });
